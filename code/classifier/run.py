...
import os
import sys
import code2nl_cross_classifier.bleu as bleu
import pickle
import torch
import json
import random
...
from code2nl_cross_classifier.model import Seq2Seq
...
def convert_examples_to_features(examples, tokenizer, args,stage=None):
    ...
    for example_index, example in enumerate(examples):
        ...
        extra_label = example.extra_label
...
def main():
    ...
    parser.add_argument("--output_dir", default=root+'/output/test_training/', type=str,
                        help="The output directory where the model predictions and checkpoints will be written.")
    parser.add_argument("--load_model_path", default=None, type=str, 
                        help="Path to trained model: Should contain the .bin files" )    
    ## Other parameters
    parser.add_argument("--train_filename", default=train_dir+'test.jsonl', type=str,
                        help="The train filename. Should contain the .jsonl files for this task.")
    parser.add_argument("--dev_filename", default=None, type=str,
                        help="The dev filename. Should contain the .jsonl files for this task.")
    parser.add_argument("--test_filename", default=train_dir+'valid.jsonl', type=str,
    ...
    if args.do_train:
        ...
        extra_label = torch.tensor([int(f.extra_label) for f in train_features], dtype=torch.long)
        train_data = TensorDataset(all_source_ids,all_source_mask,all_source_ids_2,all_source_mask_2,
                                   all_target_ids,all_target_mask,extra_label)
        ...
        for idx, file in enumerate(files):
            ...
            all_extra_labels = torch.tensor([int(f.extra_label) for f in eval_features], dtype=torch.long)
        ...
        for epoch in range(args.num_train_epochs):
            ...
            for batch in bar:
                ...
                source_ids,source_mask,source_ids_2,source_mask_2,target_ids,target_mask,extra_label = batch
                loss,_,_ = model(source_ids=source_ids,source_mask=source_mask,
                                 source_ids_2=source_ids_2,source_mask_2=source_mask_2,
                                 target_ids=target_ids,target_mask=target_mask,extra_labels=extra_label)
            ...
            for batch in tqdm(eval_dataloader, total=len(eval_dataloader)):
                batch = tuple(t.to(device) for t in batch)
                source_ids, source_mask, source_ids_2, source_mask_2, extra_labels = batch
                with torch.no_grad():
                    loss, patch_output = model(source_ids=source_ids, source_mask=source_mask,
                                               source_ids_2=source_ids_2, source_mask_2=source_mask_2,
                    ...
                    if final_preds is None:
                        final_preds = patch_output.detach().cpu().numpy()
                    else:
                        final_preds = np.append(final_preds, patch_output.detach().cpu().numpy(), axis=0)
            model.train()

            final_preds = np.argmax(final_preds, axis=1)
            eva_result = {"report": classification_report(all_extra_labels.detach().cpu().numpy(), final_preds)}
            f1_tmp = eva_result['report'].split('macro avg')[-1].split()
            f1_tmp = float(f1_tmp[2])
            if f1_tmp > best_f1:
                best_f1 = f1_tmp
                with open(os.path.join(args.output_dir, "test_{}.output".format(str(0) + "_" + str(4))), 'w',
                          encoding='utf-8') as f, open(
                    os.path.join(args.output_dir, "test_{}.gold".format(str(0) + "_" + str(4))), 'w',
                    encoding='utf-8') as f1:
                    for ref, gold in zip(final_preds, eval_examples):
                        f.write(str(gold.idx) + '\t' + str(ref) + '\n')
                        f1.write(str(gold.idx) + '\t' + gold.extra_label + '\n')
                with open(os.path.join(args.output_dir, 'eva_result'), 'w', encoding='utf-8') as f:
                    f.write(str(eva_result))
    ...
    if args.do_eval:
        ...
        for idx,file in enumerate(files):   
            ...
            all_source_mask_2 = torch.tensor([f.source_mask_2 for f in eval_features], dtype=torch.long)
            all_extra_labels = torch.tensor([int(f.extra_label) for f in eval_features], dtype=torch.long)
            eval_data = TensorDataset(all_source_ids,all_source_mask, all_source_ids_2, all_source_mask_2,all_extra_labels)
            ...
            for batch in tqdm(eval_dataloader,total=len(eval_dataloader)):
                ...
                source_ids,source_mask,source_ids_2,source_mask_2,extra_labels= batch
                with torch.no_grad():
                    loss, patch_output = model(source_ids=source_ids,source_mask=source_mask,source_ids_2=source_ids_2,source_mask_2=source_mask_2,extra_labels=extra_labels)
                    # for pred in preds:
                    #     t=pred[0].cpu().numpy()
                    #     t=list(t)
                    #     if 0 in t:
            ...
            final_preds = np.argmax(final_preds, axis=1)
            eva_result = {"report": classification_report(all_extra_labels.detach().cpu().numpy(), final_preds)}
            with open(os.path.join(args.output_dir, "test_{}.output".format(str(idx) + "_" + str(4))), 'w',
                      encoding='utf-8') as f, open(
                os.path.join(args.output_dir, "test_{}.gold".format(str(idx) + "_" + str(4))), 'w',
                encoding='utf-8') as f1:
                for ref, gold in zip(final_preds, eval_examples):
                    f.write(str(gold.idx) + '\t' + str(ref) + '\n')
                    f1.write(str(gold.idx) + '\t' + gold.extra_label + '\n')
